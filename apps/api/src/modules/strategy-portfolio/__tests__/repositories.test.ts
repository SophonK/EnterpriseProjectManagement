...
describe("ProjectAlignmentViewRepository", () => {
  ...
  it("aggregateByPortfolio — groups by portfolioId with count + summed budget (null→0)", async () => {
    ...
    const groups = await repo.aggregateByPortfolio();
  ...
  it("aggregateByGoal — per-link expansion, null budget treated as 0", async () => {
    ...
    const groups = await repo.aggregateByGoal();
